package com.boyasafe.commons.shiro.captcha;
...
import com.boyasafe.commons.utils.WebUtils;
...
public class DreamCaptcha implements InitializingBean {
	...
	public void generate(HttpServletRequest request, HttpServletResponse response) {
		...
		String captchaCode = boyasafe.generateCode().toUpperCase();// 转成大写重要
		// 不存在cookie时设置cookie
		if (!hasCookie) {
			WebUtils.setCookie(response, cookieName, cookieValue, DEFAULT_MAX_AGE);
		}
		// 生成验证码
		boyasafe.generate(response, captchaCode);
